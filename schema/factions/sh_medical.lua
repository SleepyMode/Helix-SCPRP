
FACTION.name = "Medical Staff"
FACTION.description = "People in charge of handling medical emergencies within the facility."
FACTION.color = Color(220, 30, 190)
FACTION.isDefault = true

FACTION.models = {
	"models/humans/group01/scrub1a.mdl",
	"models/humans/group01/scrub1b.mdl",
	"models/humans/group01/scrub1c.mdl",
	"models/humans/group01/scrub1d.mdl",
	"models/humans/group01/scrub1e.mdl",
	"models/humans/group01/scrub1f.mdl",
	"models/humans/group01/scrub1g.mdl",
	"models/humans/group01/scrub1h.mdl",
	"models/humans/group01/scrub1i.mdl",
	"models/humans/group01/scrub2a.mdl",
	"models/humans/group01/scrub2b.mdl",
	"models/humans/group01/scrub2c.mdl",
	"models/humans/group01/scrub2d.mdl",
	"models/humans/group01/scrub2e.mdl",
	"models/humans/group01/scrub2f.mdl",
	"models/humans/group01/scrub2g.mdl",
	"models/humans/group01/scrub2h.mdl",
	"models/humans/group01/scrub2i.mdl",
	"models/humans/group01/scrubfe1a.mdl",
	"models/humans/group01/scrubfe1b.mdl",
	"models/humans/group01/scrubfe1c.mdl",
	"models/humans/group01/scrubfe1d.mdl",
	"models/humans/group01/scrubfe1e.mdl",
	"models/humans/group01/scrubfe2a.mdl",
	"models/humans/group01/scrubfe2b.mdl",
	"models/humans/group01/scrubfe2c.mdl",
	"models/humans/group01/scrubfe2d.mdl",
	"models/humans/group01/scrubfe2e.mdl",
	"models/humans/group01/scrubfe2f.mdl"
}

FACTION_MEDIC = FACTION.index